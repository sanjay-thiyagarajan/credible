...
def histo_graph(result):
  fig = go.Figure(data=go.Scatter(y=[1,2,3,4,5],x=result))
  fig.update_layout(yaxis_title='Ratings',xaxis_title='No.of.Users')
  fig.show()
...
if __name__=='__main__':
  result = fetchreviews('com.innersloth.spacemafia&hl=en_IN&gl=US')
  cleaned_reviews = [clean_review(review) for review in result['comments']]
  summary = '. '.join(cleaned_reviews)
  Review_pie(cleaned_reviews)
  histo_graph(result['histogram'])
  pie_chart('behavioral-traits',summary[5333:15333])
  pie_chart('emotional-traits',summary[5333:15333])
