...
def analysis(request):
    if request.method == 'POST':
        ...
        if input_text != '':
            gauge_div = sentiment_gauge(input_text[:10000])
            bt_div = pie_chart('behavioral-traits', input_text[:10000])
            em_div = pie_chart('emotional-traits', input_text[:10000])
            ip_div = pie_chart('iptc', input_text[:10000])
